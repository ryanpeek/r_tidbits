# see here for this code source: https://waterdata.usgs.gov/blog/furthest-water/

library(sf)
library(nhdplusTools)
library(dplyr)

scenario = "waterbodies"

# scenario = c("waterbodies", "remove_intermittent", "filter_monthly_flow")

db <- "/Users/ryanpeek/Downloads/data_temp/NHDPlusV21_National_Seamless_Flattened_Lower48.gdb"

st_layers(db)

furthest_water <- function(scenario) {
  # First we will use a nhdplusTools to load up the national seamless geodatabase.
  nhdplusTools::nhdplus_path(db)
  
  staged_data <- nhdplusTools::stage_national_data(include = "flowline", 
                                                   output_path = "/Volumes/RAP200/nhdplus_seamless/staged_national_data")
  
  #flowlines <- readRDS(staged_data$flowline)
  flowlines <- readRDS("/Volumes/RAP200/nhdplus_seamless/staged_national_data/nhdplus_flowline.rds")
  ...
  # }
  # save out for quicker access
  #saveRDS(water_bodies, file = "/Volumes/RAP200/nhdplus_seamless/seamless/staged_national_data/nhdplus_water_bodies.rds")
  
  if("waterbodies" %in% scenario) {
    water_bodies <- readRDS("/Volumes/RAP200/nhdplus_seamless/staged_national_data/nhdplus_water_bodies.rds")
  }
  ...
  states <- read_sf("/Volumes/RAP200/GIS/states_21basic/states.shp")
  ...
  flowlines <- flowlines %>%
    ...
    #st_simplify(preserveTopology=TRUE, 1000) %>%
