# wavemaker
# see awesome post and repo by Meghan Harris
# git: https://github.com/Meghansaha/Data_Art/blob/master/creations/Functions/Waves/Making_Waves.R
# https://thetidytrekker.com/post/making-waves/making-waves.html
...
wave_layers <- wave_maker(12, wave)
...
# virids
colors_pal <- sample(viridis::viridis(15,option = "G"), nrow(wave_layers)/nrow(wave), replace = TRUE)
...
wave_layers %>%
  ggplot(aes(x=x*(y-1e4),y=y^2, group = group))+
  theme_void()+
  theme(plot.background = element_rect(fill = "#ffffff"))+
  geom_polygon(color = "black",
               size = 2,
